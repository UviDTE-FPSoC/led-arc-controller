...
extern uint8_t RXBuffer[500];
uint8_t pixel[500];
extern uint8_t TXBuffer[100];
socketState_t socket_state;
uint16_t rxLen;
uint8_t error=0, mensaje;
uint8_t dir_slave[]={0x0C,0x0F}, dir_gen=0x00;
uint8_t direccion, timer_type;
uint16_t contador=0;
uint8_t estado_t=0;
uint8_t trigger;
...
uint8_t s=0;
uint8_t n=0;
uint16_t n_led, led_group, led_env=0;
uint16_t est_sec, est_sec_r=0;
uint32_t periodo, rep, rep_r=0, tiempo=0, tiempo1=0;
...
void arc_config(void){
    ...
    if(arc_tar==0){
        ...
        for(uint32_t i=0;i<led_group*est_sec;i++){
...
void __attribute__((picinterrupt(("high_priority")))) interrupciones(void){
    ...
    if(PIR0bits.IOCIF==1 && PIE0bits.IOCIE==1){
        PIR0bits.IOCIF=0;
    ...
    if (PIR3bits.SSP1IF==1 && PIE3bits.SSP1IE==1){
        PIR3bits.SSP1IF=0;
        switch(spi_estado){
            case 0:
                start_frame();
