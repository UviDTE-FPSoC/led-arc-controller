...
error_msg ICMP_Receive(ipv4Header_t *ipv4Hdr)
{
    ...
    error_msg ret = ERROR;
    BufferReadBlock(&dataBuffers.rxBuffer, &icmpHdr, sizeof(icmpHeader_t));
    ...
    switch(((((uint16_t)(icmpTypeCodes_t)icmpHdr.typeCode & (uint16_t)0xFF00) >> 8) | (((uint16_t)(icmpTypeCodes_t)icmpHdr.typeCode & (uint16_t)0x00FF) << 8)))
    {
        ...
        case DEST_PORT_UNREACHABLE:
            BufferDump(&dataBuffers.rxBuffer, 4);
            BufferReadBlock(&dataBuffers.rxBuffer, ipv4Hdr, sizeof(ipv4Header_t));
            if(5 == ipv4Hdr->ihl)
            {
                portUnreachable = BufferRead16(&dataBuffers.rxBuffer);
            }
            break;
        default:
            break;
...
error_msg ICMP_EchoReply(ipv4Header_t *ipv4Hdr)
{
    ...
    identifier = BufferRead16(&dataBuffers.rxBuffer);
    sequence = BufferRead16(&dataBuffers.rxBuffer);
    ...
    if(ret == SUCCESS)
    {
        ...
        BufferWrite16(&dataBuffers.txBuffer, ECHO_REPLY);
        BufferWrite16(&dataBuffers.txBuffer, 0);

        BufferWrite16(&dataBuffers.txBuffer, identifier);
        BufferWrite16(&dataBuffers.txBuffer, sequence);
        ETH_SaveRDPT();

        ret = BufferRXtoTXCopy(&dataBuffers.rxBuffer, &dataBuffers.txBuffer, ipv4PayloadLength - sizeof(icmpHeader_t) - 4);
        if(ret==SUCCESS)
        {
            ...
            BufferReplaceAtOffset(&dataBuffers.txBuffer, (char *)&cksm, sizeof(cksm), sizeof(ethernetFrame_t) + sizeof(ipv4Header_t) + ((size_t)(((char*)&((icmpHeader_t *)1)->checksum)-1)));
...
error_msg ICMP_PortUnreachable(uint32_t srcIPAddress,uint32_t destIPAddress, uint16_t length)
{
    ...
    if(ret == SUCCESS)
    {
        BufferWrite16(&dataBuffers.txBuffer, DEST_PORT_UNREACHABLE);
        BufferWrite16(&dataBuffers.txBuffer, 0);
        BufferWrite32(&dataBuffers.txBuffer, 0);
        ETH_SetReadPtr(Network_GetStartPosition());
        ETH_SaveRDPT();
        BufferRXtoTXCopy(&dataBuffers.rxBuffer, &dataBuffers.txBuffer, sizeof(ipv4Header_t) + length);
