...
static error_msg TCP_Snd(tcpTCB_t *tcbPtr)
{
    ...
    if (ret == SUCCESS)
    {
        BufferWriteBlock(&dataBuffers.txBuffer, (char *) &txHeader, sizeof(tcpHeader_t));

        if (tcpDataLength > 0)
        {
            BufferWriteBlock(&dataBuffers.txBuffer, (char *) data, tcpDataLength);
        }

        cksm = payloadLength + TCP_TCPIP;

        cksm = ETH_TxComputeChecksum(sizeof(ethernetFrame_t) + sizeof(ipv4Header_t) - 8, payloadLength + 8, cksm);
        BufferReplaceAtOffset(&dataBuffers.txBuffer, (char *)&cksm, 2, sizeof(ethernetFrame_t) + sizeof(ipv4Header_t) + ((size_t)(((char*)&((tcpHeader_t *)1)->checksum)-1)));
...
error_msg TCP_PayloadSave(uint16_t len)
{
    ...
    if (currentTCB->rxBufState == RX_BUFF_IN_USE)
    {
        ...
        BufferReadBlock(&dataBuffers.rxBuffer, currentTCB->rxBufferPtr, buffer_size);
...
static error_msg TCP_ParseTCPOptions(void)
{
    ...
    if (tcpOptionsSize > 0)
    {
        ...
        if(tcpHeader.syn)
        {
            ...
            {
                opt = BufferRead8(&dataBuffers.rxBuffer);
                switch (opt)
                {
                    case TCP_EOP:
                        ...
                        if (tcpOptionsSize)
                        {
                            ...
                            BufferDump(&dataBuffers.rxBuffer, tcpOptionsSize);
                            tcpOptionsSize = 0;
                        }
                        ret = SUCCESS;
                        break;
                    ...
                    case TCP_MSS:
                        if (tcpOptionsSize >= 3)
                        {
                            ...
                            if (opt == 0x04)
                            {
                                ...
                                tcpMss = BufferRead16(&dataBuffers.rxBuffer);
                    ...
                    default:
                        ...
                        if (opt > 1)
                        {
                            ...
                            if (opt <= tcpOptionsSize)
                            {
                                ...
                                BufferDump(&dataBuffers.rxBuffer, opt);
...
void TCP_Recv(uint32_t remoteAddress, uint16_t length)
{
    ...
    BufferReadBlock(&dataBuffers.rxBuffer, (char *)&tcpHeader, sizeof(tcpHeader_t));
